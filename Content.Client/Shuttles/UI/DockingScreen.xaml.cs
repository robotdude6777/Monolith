...
public sealed partial class DockingScreen : BoxContainer
{
    ...
    public event Action<List<NetEntity>>? ToggleFTLLockRequest;
    ...
    public DockingScreen()
    {
        ...
        FTLLockButton.OnPressed += _ => OnFTLLockPressed();
    }
    ...
    private void OnFTLLockPressed()
    {
        ...
        // Always invoke the event, even if there are no docked shuttles
        // The server-side will handle the current entity if there are no docked shuttles
        ToggleFTLLockRequest.Invoke(dockedShuttles);
    }
    ...
    private void UpdateFTLLockButton(EntityUid? shuttle)
    {
        ...
        if (anyDisabled)
        {
            // Some locks are disabled
            FTLLockButton.StyleClasses.Add("OpenRight");
            FTLLockButton.StyleClasses.Remove("Average");
        }
        else
        {
            // All locks are enabled (or no components found)
            FTLLockButton.StyleClasses.Add("Average");
            FTLLockButton.StyleClasses.Remove("OpenRight");
        }
    }
